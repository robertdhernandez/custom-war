...
class ConsoleFunctionException : public std::exception
{
	std::string m_err;
	
public:
	ConsoleFunctionException( const std::string & msg ) throw()
	{
		m_err = msg;
	}
	
	~ConsoleFunctionException() throw() {}
...
static class HELP : public con::Command
{
	...
	void execute( Console& c, const Arguments& args )
	{
		...
		else
		{
			...
			if ( find == cmds.end() )
				throw ConsoleFunctionException( "Unknown console command" );
			(*find)->help( c );
		}
	}
} HELP;
...
LevelEditor& getLevelEditor()
{
	...
	if ( ret == nullptr )
		throw ConsoleFunctionException( "Must be in level editor" );
	return *ret;
}
...
net::NetBase& getNetBase()
{
	...
	if ( ret == nullptr )
		throw ConsoleFunctionException( "Network commands are currently unavailable" );
	return *ret;
}
...
net::Host& getHost()
{
	...
	if ( ret == nullptr )
		throw ConsoleFunctionException( "Host commands are currently unavailable" );
	return *ret;
}
...
net::Client& getClient()
{
	...
	if ( ret == nullptr )
		throw ConsoleFunctionException( "Client commands are currently unavailable" );
	return *ret;
}
